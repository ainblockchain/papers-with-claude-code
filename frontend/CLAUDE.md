...
- **Auth**: NextAuth v5 (JWT strategy, GitHub OAuth)
- **State**: Zustand (useAuthStore, usePurchaseStore)
- **Blockchain**: AIN devnet (`@ainblockchain/ain-js`)
- **Payment**: x402 protocol on Base Mainnet (CDP facilitator)
...
`/login` → GitHub OAuth → Passkey 등록/인증 → Identity sync (블록체인) → `/explore`
