...
② **문제점 구체화** — 자유 문장이 "무엇이 어떻게 문제인지" 를 드러냄.
   - 구체적 표현 (통과): "잘못 매칭", "오분류", "엉뚱한 답변", "질문 의도와 다른 답", "{유저 주제}인데 {현재 라벨}로 잡힘", "{유저 주제} 질문인데 {현재 라벨} 답변"
...
[모범 답안 — 반드시 pass]
"""
유저가 물은 주제는 A 인데, 챗봇이 B 인텐트(=현재 라벨 "${intent}")로 잘못 매칭됐다.
[표 로그]
"""
자유 문장에 (유저 원문의) 주제 + 현재 라벨 + 구체적 문제("잘못 매칭"/"오분류") 모두 있음.

[추가 통과 예시 — 라벨명·주제명은 이 케이스의 값으로 대입해서 보세요]
- "{현재 라벨} 인텐트로 잘못 매칭됨" ✓
- "{유저 주제} 질문인데 엉뚱한 답변" ✓
- "{유저 주제} 아닌데 {현재 라벨}로 잡힘" ✓
- "{유저 주제} 관련 질문인데 {현재 라벨}로 분류. 의도 안 맞음." ✓
